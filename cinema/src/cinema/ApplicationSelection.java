...
//        }
        ...
        List<Video> l = em.createQuery("SELECT v FROM Video v", Video.class).getResultList();
        for (Video v : l) System.out.println(v);
        ...
        List<Artiste> res4 = em.createQuery(
                "SELECT DISTINCT v.realisateur FROM Video v WHERE v.realisateur.anneeNaissance > 800",
                Artiste.class
        ).getResultList();
        ...
        List<Object[]> res5 = em.createQuery(
                "SELECT p, COUNT(fdp) FROM Pays p LEFT JOIN p.filmsDuPays fdp WHERE TYPE(fdp) = Film GROUP BY p",
                Object[].class
        ).getResultList();
